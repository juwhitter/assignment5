...
public class TowersOfHanoi
{
    ...
    public void moveDiscSingle (char from, char to) throws Exception
    {
    	...
    	//if anything besides a b or c is sent in through from or to, will throw the exception.
    	if ((from != 'A' && from != 'B' && from != 'C') || (to != 'A' && to != 'B' && to !='C'))
    		throw new Exception("Abort! Illegal tower char ID!");
    	
    	//Creates local  tower objects that reference the outside towers depending on the char sent in, runs after the valid input check.
    	MyStack<Integer> fromTower;
    	MyStack<Integer> toTower;
    	...
    	//If the stack referenced by fromTower is empty, returns an error.
    	if (fromTower.isEmpty())
    		throw new Exception("Abort! Illegal tower request!");
  
  	
    	//If the toTower isn't empty and the top of the from tower is greater than the top of the to tower, throws an exception.
    	if ((!(toTower.isEmpty()) && fromTower.peek() >= toTower.peek()))
    		throw new Exception("Abort! Illegal disc movement!");
    		
    	
    	// Move one disc from the "from" tower to the "to" tower  
    	int transfer = fromTower.pop();
    	toTower.push(transfer);
